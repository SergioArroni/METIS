"""
Result data structure for coverage metrics aggregation.
"""
...
@dataclass
class CoverageResult(AggregationResult):
    """
    Result of coverage aggregation.

    Extends the base AggregationResult with coverage-specific functionality.
    ...
    def to_metric_result(self) -> MetricResult:
        ...
        return MetricResult(
            id="fidelity.marginal.coverage",
            value=self.score,
            details=details,
            family="fidelity",
            purpose_tags={"fidelity", "marginal", "coverage", "distribution"},
        )

    def get_report_data(self) -> dict[str, Any]:
        """Get structured data for report generation."""
        return super().get_report_data(metric_id="fidelity.marginal.coverage", category="cobertura")

    @classmethod
    def empty(cls, metrics_used: list[str]) -> "CoverageResult":
        """
